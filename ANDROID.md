...
| 编号 | Android 图形功能 | 行为 |
|---|---|---|
| 1 | 安装/升级/自适应优化 | 唯一安装入口；缺失则安装、旧版则升级、同一完整构建跳过上传，新于客户端则拒绝降级；随后手填域名和邮箱、选 1—15/R/A 模板、以系统 DNS 或 Cloudflare+Google 公共 DNS 双重确认解析、执行施工、验证交接单，可选整理备份和打开面板。 |
...
| 7 | 凭据交接单 | 只有 VPS 账户/密码和面板账户/密码四项都是真实非占位值时才显示、复制；空输出、残缺表和 `Connection closed` 一律失败关闭。 |
