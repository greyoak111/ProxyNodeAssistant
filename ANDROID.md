# ProxyNodeAssistant v1.0.0 Android 完整使用说明
...
最低 Android 7.0（API 24），面向 Android 16（target API 36）。APK 是 universal 包，可安装在常见 ARM64、ARMv7 和 x86_64 Android 设备上；SSH 部分为纯 Java/Kotlin，不要求 root。本次修复版显示版本 `1.0.0`、内部构建修订 `113`、`versionCode=1000001`，并保持原 applicationId；递增 versionCode 可覆盖安装先前 Android 正式构建。
...
1. 把 `ProxyNodeAssistant-v1.0.0-android-universal.apk` 传到手机。
...
每个远端操作都会打开目标与认证页，并自动载入最近一次使用的非敏感目标（主机/IP、SSH 用户、端口）供确认和修改；没有历史时保持空白。载入目标不会自动执行操作。

- `节点长期 KEY`：按 `SSH用户@主机:端口` 独立查找。有匹配且已绑定的 key 时才默认选中；没有绑定 key 时必须明确改选 `临时密码`，认证成功后再询问是否绑定。
- `临时密码`：必须在本次表单明确选择；密码只用于当前 SSH 认证，不写日志、不进剪贴板、不写磁盘。
...
最近目标只记录主机名/IP、SSH 用户、端口、标签和最后使用时间，不记录密码。打开标准远端表单时会自动载入最近目标，但字段仍可编辑；可在“节点”页单独删除或全部清空。
...
| 5 | VPS 密码轮换 | 先选 `1` 随机、`2` 自定义或 `0` 取消；自定义密码遮罩输入并二次确认，验证交接单后才允许显示/复制。 |
| 6 | 3x-ui 身份轮换 | 先选 `1` 随机、`2` 自定义或 `0` 取消；自定义账号/密码遮罩输入并二次确认，验证交接单后才允许显示/复制；会注销旧会话，可能影响原有 2FA。 |
| 7 | 凭据交接单 | 只接受含运行标记且至少有一个有效字段的结构化交接单；空输出和 `Connection closed` 会失败关闭。 |
| 8 | 伪装站/Nginx | 列出 15 套内置模板，支持随机、按域名稳定或指定编号。 |
| 9 | 完整灾备 | 生成包含程序/节点配置的较大备份；不生成本机 admin、设备身份或准入材料。 |
...
菜单 `[5]` 和 `[6]` 的自定义秘密只存在于当前运行：应用通过已认证 SSH 会话写入随机命名、root-only、权限 `0600` 的一次性文件，远端脚本读取后立即删除；成功、失败、取消或 Android 协程取消都会清理。秘密不会写入设置、日志、命令行或 APK。选择 `0` 直接返回且不修改 VPS。
...
5. 最终预览会显示路线、隐去部分的邮箱、模板、性能、WARP 和端口：Reality `443`、验货 `24443`、CDN 源站 `8443`、WARP 本地 `40000`、SS2022 TCP `32443`。`30443` 只作为已有 trial/迁移节点的兼容端口，升级时检测到既有监听会默认保留，不会静默改端口。
6. 只有输入大写 `APPLY` 才会上传工具包或修改 VPS；其他输入一律取消。

施工参数写入每次随机命名的 `/tmp/proxy-node-assistant-*.env`，权限固定为 `0600`，远端读取后立即删除，失败或取消也会清理。客户端不设置 `PROXY_RUNBOOK_ASSUME_DEFAULTS=1`，也不使用固定的共享输入路径。橙云/双路在最终切换前还必须完成 Cloudflare 人工确认和真实客户端浏览验货，验货拒绝或失败会回滚待提交拓扑。
...
发布 APK 和源码不得包含真实 VPS IP、域名、邮箱、密码、API Key、Token 或私钥。APK 内资产名固定为 `proxy-node-assistant-toolkit-v1.0.0.tgz`（上传到远端时恢复标准 `.tar.gz` 名），构建脚本会校验它与仓库当前正式工具包 SHA-256 完全一致；发布前还应对源码、解包 APK 和内嵌 tar 做二次扫描。
