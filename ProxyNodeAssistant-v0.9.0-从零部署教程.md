# ProxyNodeAssistant v0.9.0 从零部署教程

适用对象：第一次购买 VPS、第一次购买域名、第一次使用 Cloudflare DNS 的用户  
适用客户端：Windows v0.9.0、Android v0.9.0  
教程核对日期：2026-08-23
...
- 一条指向 VPS 的灰云 A 记录；
- 可用的 SSH 登录方式；
- 由 ProxyNodeAssistant v0.9.0 部署并验证过的节点、伪装站、订阅和本地面板隧道；
...
~~~text
ProxyNodeAssistant-v0.9.0-win64.exe      绝大多数 Intel/AMD Windows 电脑
ProxyNodeAssistant-v0.9.0-win32.exe      仅 32 位 Windows 10
ProxyNodeAssistant-v0.9.0-win-arm64.exe  Windows on ARM
~~~

Android 使用 v0.9.0 universal APK。Android 执行远端操作前，应暂停会把 SSH 也绕回目标 VPS 的全局 VPN，避免连接自环。

### 8.2 校验下载文件

在发布目录下载 SHA256SUMS-v0.9.0.txt，然后在 PowerShell 执行：
...
- v0.9.0 的服务商 API Key/Token 只能临时输入，或经确认存进系统安全存储，不能写进教程、日志或仓库。
...
- [ProxyNodeAssistant 完整使用说明书](ProxyNodeAssistant-v0.9.0-完整使用说明书.md)
