...
BASELINE="$ROOT/runbook/text-node-assistant-v0.9.5/linux/24-security-baseline.sh"
EVENTS="$ROOT/runbook/text-node-assistant-v0.9.5/linux/25-security-events.sh"
...
DISMANTLE="$ROOT/runbook/text-node-assistant-v0.9.5/linux/22-dismantle-managed-node.sh"

for file in "$BASELINE" "$EVENTS" "$NGINX" "$DIAG" "$DISMANTLE"; do
  bash -n "$file"
done

grep -q 'TNA_MANAGED_FAIL2BAN_SSHD_V095' "$BASELINE"
...
grep -q '__TNA_SECURITY_V1_BEGIN__' "$EVENTS"
grep -q '__TNA_SECURITY_V1_END__' "$EVENTS"
grep -q 'source_state .* PARTIAL' "$EVENTS"
grep -q 'TNA-REALITY' "$BASELINE"
grep -q 'RAW_SECURITY_EVENT_LOGS_EXCLUDED=1' "$ROOT/runbook/text-node-assistant-v0.9.5/linux/01-safe-backup.sh"
grep -q '24-security-baseline.sh.*--remove' "$DISMANTLE"
grep -q 'FAIL2BAN_SSHD_JAIL_ACTIVE' "$DIAG"

format_line="$(grep 'log_format tna_security' "$NGINX")"
...
grep -Fq 'route_class=\$tna_route_class' <<<"$format_line"
