...
export TNA_DEPLOYMENT_STATE_FILE="$TMP/etc/deployment-state.env"
export TNA_DEPLOYMENT_LOCK_FILE="$TMP/run/deployment.lock"
...
. "$ROOT/runbook/text-node-assistant-v0.9.5/linux/lib-deployment-state.sh"

tna_state_init_direct_if_missing
state="$(tna_state_show)"
...
if tna_state_transition CDN_STAGED_8443 ACTIVE_CDN cdn-xhttp-tls nginx-cdn previously-exposed 2>/dev/null; then
  echo 'unsafe transition was accepted' >&2
  exit 1
fi
grep -q '^ACTIVE_MODE=CDN_STAGED_8443$' <<<"$(tna_state_show)"
