...
Assert-Match $workflow 'const val BUILD_REVISION = 111' "Android workflow revision is not 111"
Assert-Match $workflow 'const val BUILD_ID = "20260901-v100-ss2022-r111"' "Android workflow build id is not revision 111"
...
Assert-Match $appUi 'BUILD 1\.0\.0-R111 / ANDROID' "Android UI build label is not revision R111"
...
Assert-Match $androidBuilder '20260901-v100-ss2022-r111' "Android build no longer verifies the exact toolkit build id"
Assert-Match $androidBuilder '\$archiveBuildRevision -ne "111"' "Android build no longer verifies toolkit revision 111"
...
if ($archiveVersion -ne '1.0.0' -or $archiveBuildId -ne '20260901-v100-ss2022-r111' -or $archiveRevision -ne '111') {
    throw "Embedded toolkit metadata is not the exact v1.0.0 revision-111 build"
}
