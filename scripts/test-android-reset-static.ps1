...
Assert-Match $workflow 'const val BUILD_REVISION = 109' "Android workflow revision is not 109"
Assert-Match $workflow 'const val BUILD_ID = "20260901-v100-ss2022-r109"' "Android workflow build id is not revision 109"
...
Assert-Match $appUi 'BUILD 1\.0\.0-R108 / ANDROID' "Android UI build label is not revision R108"
...
Assert-Match $androidBuilder '20260901-v100-ss2022-r109' "Android build no longer verifies the exact toolkit build id"
Assert-Match $androidBuilder '\$archiveBuildRevision -ne "109"' "Android build no longer verifies toolkit revision 109"
...
if ($archiveVersion -ne '1.0.0' -or $archiveBuildId -ne '20260901-v100-ss2022-r109' -or $archiveRevision -ne '109') {
    throw "Embedded toolkit metadata is not the exact v1.0.0 revision-109 build"
}
