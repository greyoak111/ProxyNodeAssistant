...
Assert-Match $workflow 'const val BUILD_REVISION = 108' "Android workflow revision is not 108"
Assert-Match $workflow 'const val BUILD_ID = "20260901-v100-ss2022-r108"' "Android workflow build id is not revision 108"
...
Assert-Match $androidBuilder '20260901-v100-ss2022-r108' "Android build no longer verifies the exact toolkit build id"
Assert-Match $androidBuilder '\$archiveBuildRevision -ne "108"' "Android build no longer verifies toolkit revision 108"
...
if ($archiveVersion -ne '1.0.0' -or $archiveBuildId -ne '20260901-v100-ss2022-r108' -or $archiveRevision -ne '108') {
    throw "Embedded toolkit metadata is not the exact v1.0.0 revision-108 build"
}
