...
Assert-Match $workflow 'const val BUILD_REVISION = 113' "Android workflow revision is not 113"
Assert-Match $workflow 'const val BUILD_ID = "20260901-v100-ss2022-r113"' "Android workflow build id is not revision 113"
...
Assert-Match $appUi 'BUILD 1\.0\.0-R113 / ANDROID' "Android UI build label is not revision R113"
...
Assert-Match $androidBuilder '20260901-v100-ss2022-r113' "Android build no longer verifies the exact toolkit build id"
Assert-Match $androidBuilder '\$archiveBuildRevision -ne "113"' "Android build no longer verifies toolkit revision 113"
...
if ($archiveVersion -ne '1.0.0' -or $archiveBuildId -ne '20260901-v100-ss2022-r113' -or $archiveRevision -ne '113') {
    throw "Embedded toolkit metadata is not the exact v1.0.0 revision-113 build"
}
