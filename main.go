...
func (a *App) printMenu() {
	if a.lang == LangZH {
		...
		a.println("[14] 本地 10808 代理：接管/恢复 macOS 系统代理并检测 TUN（不管理第三方 TUN，不连接 VPS）")
		...
		a.println("[14] Local 10808 proxy: manage/restore macOS system proxy and inspect TUN (never manages third-party TUN; no VPS login)")
