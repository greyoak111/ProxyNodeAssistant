...
$archiveBuildRevision = (& tar -xOf $toolkitSource "proxy-node-assistant-v1.0.0/TOOLKIT_BUILD_REVISION" | Out-String).Trim()
    if ($archiveVersion -ne "1.0.0" -or $archiveBuildId -ne "20260901-v100-ss2022-r113" -or $archiveBuildRevision -ne "113") {
        throw "Current toolkit archive metadata is not the exact ProxyNodeAssistant v1.0.0 revision-113 build"
}
