...
set "OUTPUT_DIR=%SOURCE_DIR%outputs\ProxyNodeAssistant-v1.0.0-official"

echo ============================================================
echo  ProxyNodeAssistant v1.0.0 reset line - Windows 全架构正式打包
echo ============================================================
echo  源码目录: %SOURCE_DIR%
echo  输出目录: %OUTPUT_DIR%
echo.
echo  将生成:
echo    1. Windows x64   - ProxyNodeAssistant-v1.0.0-win64.exe
echo    2. Windows x86   - ProxyNodeAssistant-v1.0.0-win32.exe
echo    3. Windows ARM64 - ProxyNodeAssistant-v1.0.0-win-arm64.exe
echo    4. macOS/Linux CLI - amd64 + arm64
echo.
...
echo.
echo [4/6] 交叉编译 macOS/Linux CLI...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%SOURCE_DIR%build-unix.ps1" -SkipHostTests
if errorlevel 1 goto :failed

echo.
echo [5/6] 构建并签名 Android 通用 APK...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%SOURCE_DIR%android\build-signed-release.ps1" -Provision
if errorlevel 1 goto :failed

echo.
echo [6/6] 生成便携包、源码包和 SHA-256 清单...
