...
func TestDeviceBundlesRoundTrip(t *testing.T) {
	identity := testDeviceIdentity(t)
	invite := DeviceInvite{Version: 1, NodeID: "pna-node-0123456789abcdef0123456789abcdef", Nonce: "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef", ExpiresEpoch: time.Now().Add(10 * time.Minute).Unix()}
	bundle, err := encodeDeviceBundle("PNAINV1.", invite)
	if err != nil {
		t.Fatal(err)
	}
	...
	response := DeviceEnrollmentResponse{Version: 1, NodeID: invite.NodeID, Nonce: invite.Nonce, DeviceID: identity.DeviceID, PublicKey: identity.PublicKey, Label: "Laptop 01", Role: "traffic-only"}
	seed := make([]byte, ed25519.SeedSize)
	for index := range seed {
		seed[index] = byte(index + 1)
	}
...
func TestDeviceResponseRejectsMismatchedIdentity(t *testing.T) {
	identity := testDeviceIdentity(t)
	publicBytes, _ := base64.RawURLEncoding.DecodeString(stringsTrimPrefixForTest(identity.PublicKey, "pna-ed25519:"))
	publicBytes[0] ^= 1
	response := DeviceEnrollmentResponse{
		Version: 1, NodeID: "pna-node-0123456789abcdef0123456789abcdef",
		Nonce:    "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef",
		DeviceID: identity.DeviceID, PublicKey: "pna-ed25519:" + base64.RawURLEncoding.EncodeToString(publicBytes),
		Label: "Laptop", Role: "controller",
		Signature: base64.RawURLEncoding.EncodeToString(make([]byte, ed25519.SignatureSize)),
	}
	bundle, _ := encodeDeviceBundle("PNARESP1.", response)
...
func TestParseDeviceStatus(t *testing.T) {
	text := "__PNA_DEVICE_STATUS_V1_BEGIN__\n" +
		"NODE_ID=pna-node-0123456789abcdef0123456789abcdef\n" +
		"CONTROLLER_ACTIVE_COUNT=1\nDEVICE_ACTIVE_COUNT=1\n" +
		"DEVICE\tpna-device-abcdefghijklmnopqrstuvwxyz\tcontroller\tactive\tLaptop 01\t2026-08-24T12:00:00Z\n" +
