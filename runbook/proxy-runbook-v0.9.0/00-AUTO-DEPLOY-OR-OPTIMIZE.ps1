...
$archive = Join-Path $here "proxy-runbook-toolkit-v0.9.0.tar.gz"

Write-Host "============================================================"
Write-Host " Proxy Runbook v0.9.0 - Detect-first Adaptive Installer"
Write-Host "============================================================"
Write-Host
...
& scp -i $keyPath -P $SshPort $archive "$SshUser@${VpsHost}:/tmp/proxy-runbook-toolkit-v0.9.0.tar.gz"
if ($LASTEXITCODE -ne 0) { throw "SCP upload failed." }

$isRoot = $SshUser -eq "root"
$extract = 'mkdir -p /opt; rm -rf /opt/proxy-runbook-v0.9.0; tar -xzf /tmp/proxy-runbook-toolkit-v0.9.0.tar.gz -C /opt; bash /opt/proxy-runbook-v0.9.0/linux/00-bootstrap-toolkit.sh'
$escapedUser = $SshUser.Replace("'","")

Write-Host
Write-Host "[6/7] Installing toolkit on the VPS..."
if ($isRoot) {
    & ssh -tt -i $keyPath -p $SshPort "$SshUser@$VpsHost" "env PROXY_RUNBOOK_LOGIN_USER='$escapedUser' PROXY_RUNBOOK_SSH_KEY_INSTALLED=1 bash -lc '$extract'"
} else {
    Write-Host "The VPS may ask for the sudo password of '$SshUser'." -ForegroundColor Yellow
    & ssh -tt -i $keyPath -p $SshPort "$SshUser@$VpsHost" "sudo env PROXY_RUNBOOK_LOGIN_USER='$escapedUser' PROXY_RUNBOOK_SSH_KEY_INSTALLED=1 bash -lc '$extract'"
}
...
if ($wizardExit -ne 0) {
    ...
    & ssh -T -i $keyPath -p $SshPort "$SshUser@$VpsHost" "sudo sh -c 'cat /etc/proxy-runbook/last-run.env /etc/proxy-runbook/cover-last-run.env 2>/dev/null || true'"
    exit $wizardExit
}
