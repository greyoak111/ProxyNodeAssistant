...
- 失败时读取 `/etc/proxy-runbook/last-run.env` 和 Cover 阶段状态，直接报告失败阶段。
- panel 元数据通过专用无 TTY 命令读取，并验证端口范围与 WebBasePath。
- SSH 隧道启用 `ExitOnForwardFailure=yes`，只有本地监听真正建立后才打开浏览器。

## 远端 runbook

- panel 端口/路径发现后立即原子写入 `/etc/proxy-runbook/public.env`，不再等证书、Cover、WARP 全部完成。
- 新增 `linux/18-panel-metadata.sh`，按 `public.env → x-ui setting -show → install-result.env` 回退读取。
- 自适应施工持续写入 `/etc/proxy-runbook/last-run.env`；Cover 施工另写 `/etc/proxy-runbook/cover-last-run.env`。
- 远端失败打印 `PROXY_RUNBOOK_REMOTE_FAILURE` / `PROXY_RUNBOOK_COVER_FAILURE` 阶段标记。
