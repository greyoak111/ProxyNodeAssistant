...
MARKER="proxy-runbook-cover-library-v2"
TOTAL=15
...
choose_template_id() {
  ...
  case "$choice" in
    ""|r|random)
      ...
      current="$(sed -n 's/^template_id=//p' "$ROOT/.proxy-runbook-cover" 2>/dev/null | head -n 1 || true)"
...
if [ -d "$ROOT" ] && [ -f "$ROOT/index.html" ] && [ ! -f "$ROOT/.proxy-runbook-cover" ] && ! grep -q "$MARKER" "$ROOT/index.html"; then
...
cat > "$ROOT/.proxy-runbook-cover" <<EOF
marker=$MARKER
domain=$DOMAIN
...
updated=$(date -Is)
EOF
chmod 644 "$ROOT/.proxy-runbook-cover" "$ROOT/404.html" "$ROOT/robots.txt" "$ROOT/assets/favicon.svg"
