...
echo "===== PROXY RUNBOOK TOOLKIT BOOTSTRAP ====="
echo "Toolkit: $ROOT"
...
#!/usr/bin/env bash
if [ "$(id -u)" -ne 0 ]; then
  exec sudo /opt/proxy-runbook-current/linux/13-maintenance-menu.sh "$@"
else
  exec /opt/proxy-runbook-current/linux/13-maintenance-menu.sh "$@"
fi
EOF
