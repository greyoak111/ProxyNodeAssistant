...
if [ -s /root/.config/proxy-runbook/performance-profile.env ]; then
  ...
  add_issue PERFORMANCE_UNMANAGED INFO "尚未应用 v0.9.0 可回滚性能档位；可在菜单 [16] 检测后选择" \
    "No v0.9.0 rollback-capable profile is applied; inspect and choose one from menu [16]" PERFORMANCE_PROFILE false
fi
