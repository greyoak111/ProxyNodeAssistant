...
cover_exit() {
  ...
  if [ "$rc" -ne 0 ] && [ "$COVER_COMPLETE" -ne 1 ]; then
    ...
    printf '\nPROXY_RUNBOOK_COVER_FAILURE stage=%s rc=%s\n' "$COVER_STAGE" "$rc" >&2
  fi
}
...
cover_status RUNNING 0
bash "$(dirname "$0")/05b-cover-site-polished.sh" "$DOMAIN" auto "${PROXY_RUNBOOK_COVER_TEMPLATE:-auto}" || {
  rc=$?
  if [ "$rc" -eq 20 ]; then
...
PROBE_NAME="proxy-runbook-$(openssl rand -hex 16)"
...
if [ "$LOCAL_PROBE" != "$PROBE_VALUE" ]; then
  echo "PROXY_RUNBOOK_ACME_LOCAL_PREFLIGHT_FAILURE"
...
if [ "$PUBLIC_PROBE" != "$PROBE_VALUE" ]; then
  echo "PROXY_RUNBOOK_ACME_PUBLIC_PREFLIGHT_FAILURE"
