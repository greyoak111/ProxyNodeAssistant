...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    *) printf 'TNA_SECURITY_EVENTS_ERROR=INVALID_ARGUMENT\n' >&2; exit 2 ;;
  esac
done

case "$SINCE" in 1h|6h|24h|7d) ;; *) printf 'TNA_SECURITY_EVENTS_ERROR=INVALID_SINCE\n' >&2; exit 3 ;; esac
[[ "$CURSOR" =~ ^[0-9]+$ ]] && [ "$CURSOR" -le 100000 ] || { echo 'TNA_SECURITY_EVENTS_ERROR=INVALID_CURSOR' >&2; exit 3; }
[[ "$LIMIT" =~ ^[0-9]+$ ]] && [ "$LIMIT" -ge 1 ] && [ "$LIMIT" -le 1000 ] || { echo 'TNA_SECURITY_EVENTS_ERROR=INVALID_LIMIT' >&2; exit 3; }
...
collect_kernel() {
  ...
  while IFS= read -r line; do
    ...
    elif [[ "$line" == *'TNA-REALITY '* && "$line" =~ SRC=([^[:space:]]+) ]]; then
...
collect_nginx() {
  local file=/var/log/nginx/text-node-assistant-security.log line ip epoch category
  if [ ! -e "$file" ]; then
  ...
  while IFS= read -r line; do
    ...
    case "$category" in xhttp) category=CDN_XHTTP_REQUEST ;; acme) category=ACME_REQUEST ;; *) category=WEB_REQUEST ;; esac
...
echo '__TNA_SECURITY_V1_BEGIN__'
