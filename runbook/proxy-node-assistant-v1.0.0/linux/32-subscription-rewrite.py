...
def runtime_config() -> tuple[str, str, int, str]:
    ...
    if xhttp_port != 8443:
        # 8443 is the only topology verified end-to-end for Cloudflare's
        # public edge and the restricted origin.  Refuse stale/random
        # metadata instead of exporting a link that looks valid but cannot
        # traverse the CDN.
        raise RuntimeError("XHTTP public port must be 8443")
...
def rewrite_link(link: str, xhttp_domain: str, xhttp_port: int, xhttp_path: str) -> tuple[str, bool]:
    if xhttp_port != 8443:
        raise ValueError("XHTTP public port must be 8443")
