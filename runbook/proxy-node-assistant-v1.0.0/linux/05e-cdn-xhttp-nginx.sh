...
stage() {
  ...
  if [ "$TARGET_TOPOLOGY" = dual ]; then
    reality_443_present || { echo 'TNA_CDN_NGINX_ERROR=DUAL_TARGET_REQUIRES_REALITY_443' >&2; return 118; }
    tna_state_commit_route managed-dual staging xray-reality
  else
    # Orange is not committed here. Reality remains available until the
    # caller proves the edge path and explicitly finalizes the transaction.
    if reality_443_present; then
      tna_state_commit_route managed-dual staging xray-reality
    else
      tna_state_commit_route managed-orange staging none
    fi
  fi
...
prepare_production() {
  ...
  echo 'TNA_CDN_PRODUCTION_NOT_ENABLED=WAITING_FOR_CLIENT_CONFIRMATION'
}
