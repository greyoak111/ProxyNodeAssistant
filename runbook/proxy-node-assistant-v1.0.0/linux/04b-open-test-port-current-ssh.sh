...
ufw allow from "$CLIENT_IP" to any port "$PORT" proto tcp comment 'proxy-runbook-reality-test'
ufw status numbered
echo
echo "TEST_PORT_OPEN_OK source=$CLIENT_IP port=$PORT"
echo "After 443 is verified, close this temporary rule with:"
echo "  bash /opt/proxy-runbook-current/linux/04c-close-test-port.sh"
