...
STATE_DIR="/etc/text-node-assistant/cloudflare"
STATE_FILE="$STATE_DIR/cidr-state.env"
V4_URL="https://www.cloudflare.com/ips-v4"
V6_URL="https://www.cloudflare.com/ips-v6"
ALLOW_MARKER="TNA-CF-XHTTP-V095-ALLOW"
DENY_MARKER="TNA-CF-XHTTP-V095-DENY"
MANAGED_MARKER="TNA-CF-XHTTP-V095"
MANAGED_PORT=8443
...
fetch_lists() {
  ...
  if [ "$(state_value CLOUDFLARE_FIREWALL_APPLIED || true)" = 1 ]; then
    echo 'TNA_CF_CIDR_ERROR=FIREWALL_ALREADY_APPLIED_REMOVE_BEFORE_REFRESH' >&2
    return 120
  fi
  command -v python3 >/dev/null 2>&1 || { echo 'TNA_CF_CIDR_ERROR=PYTHON3_MISSING' >&2; return 121; }
  tmp="$(mktemp -d /tmp/tna-cf-cidrs.XXXXXX)"
  v4="$tmp/ips-v4"
  v6="$tmp/ips-v6"
  if ! curl --fail --location --silent --show-error --retry 4 --connect-timeout 15 --max-time 60 --proto '=https' --tlsv1.2 --output "$v4" "$V4_URL" ||
    ...
    rm -rf -- "$tmp"
    echo 'TNA_CF_CIDR_ERROR=OFFICIAL_LIST_DOWNLOAD_FAILED' >&2
    return 121
  fi
  count4="$(validate_cidrs 4 "$v4" 10)" || { rm -rf -- "$tmp"; echo 'TNA_CF_CIDR_ERROR=IPV4_INVALID' >&2; return 122; }
  count6="$(validate_cidrs 6 "$v6" 5)" || { rm -rf -- "$tmp"; echo 'TNA_CF_CIDR_ERROR=IPV6_INVALID' >&2; return 122; }
  install -d -m 755 "$STATE_DIR"
  ...
  printf '__TNA_CF_CIDR_BEGIN__\n'
  cat "$STATE_FILE"
  printf 'CLOUDFLARE_API_MUTATION=NONE\n'
  printf '__TNA_CF_CIDR_END__\n'
}
...
print_443_observation() {
  printf 'PUBLIC_TCP_443_POLICY=UNCHANGED\n'
  if reality_443_present; then
    printf 'REALITY_443_PRESENT=1\n'
  else
    printf 'REALITY_443_PRESENT=0\n'
  fi
}

remove_managed_rules() {
  local number loops=0
  command -v ufw >/dev/null 2>&1 || { echo 'TNA_CF_LOCK_ERROR=UFW_MISSING' >&2; return 124; }
  while :; do
    number="$(managed_rule_numbers | tail -n1 || true)"
    [ -n "$number" ] || break
    ufw --force delete "$number" >/dev/null
    loops=$((loops + 1))
    [ "$loops" -le 100 ] || { echo 'TNA_CF_LOCK_ERROR=RULE_REMOVAL_GUARD' >&2; return 125; }
  done
  [ -z "$(managed_rule_numbers || true)" ] || { echo 'TNA_CF_LOCK_ERROR=MANAGED_RULES_REMAIN' >&2; return 125; }
  set_state_value CLOUDFLARE_FIREWALL_APPLIED 0
  set_state_value CLOUDFLARE_FIREWALL_REMOVED_AT "$(date -Is)"
  printf '__TNA_CF_ORIGIN_LOCK_BEGIN__\n'
  printf 'CLOUDFLARE_FIREWALL_APPLIED=0\n'
  printf 'CLOUDFLARE_FIREWALL_PORT=%s\n' "$MANAGED_PORT"
  print_443_observation
  printf 'SSH_POLICY=UNCHANGED\n'
  printf '__TNA_CF_ORIGIN_LOCK_END__\n'
}
...
apply_rules() {
  ...
  command -v ufw >/dev/null 2>&1 || { echo 'TNA_CF_LOCK_ERROR=UFW_MISSING' >&2; return 124; }
  [ -s "$STATE_DIR/ips-v4" ] && [ -s "$STATE_DIR/ips-v6" ] || { echo 'TNA_CF_LOCK_ERROR=FETCH_FIRST' >&2; return 123; }
  ufw status 2>/dev/null | grep -q '^Status: active$' || { echo 'TNA_CF_LOCK_ERROR=UFW_INACTIVE' >&2; return 124; }
  ufw status verbose 2>/dev/null | grep -q '^Default: deny (incoming)' || { echo 'TNA_CF_LOCK_ERROR=DEFAULT_INCOMING_NOT_DENY' >&2; return 124; }
  ...
  if ! ufw --force prepend deny "$MANAGED_PORT/tcp" comment "$DENY_MARKER" >/dev/null; then
    echo 'TNA_CF_LOCK_ERROR=DENY_INSERT_FAILED' >&2
    return 126
  fi
  while IFS= read -r cidr || [ -n "$cidr" ]; do
    ...
    if ! ufw --force prepend allow proto tcp from "$cidr" to any port "$MANAGED_PORT" comment "$ALLOW_MARKER" >/dev/null; then
      remove_managed_rules >/dev/null 2>&1 || true
      echo 'TNA_CF_LOCK_ERROR=IPV4_ALLOW_INSERT_FAILED_ROLLED_BACK' >&2
      return 126
    fi
  ...
  if ipv6_enabled; then
    ...
    while IFS= read -r cidr || [ -n "$cidr" ]; do
      ...
      if ! ufw --force prepend allow proto tcp from "$cidr" to any port "$MANAGED_PORT" comment "$ALLOW_MARKER" >/dev/null; then
        remove_managed_rules >/dev/null 2>&1 || true
        echo 'TNA_CF_LOCK_ERROR=IPV6_ALLOW_INSERT_FAILED_ROLLED_BACK' >&2
        return 126
      fi
  ...
  count="$(verify_rules)" || {
    remove_managed_rules >/dev/null 2>&1 || true
    echo 'TNA_CF_LOCK_ERROR=READBACK_FAILED_ROLLED_BACK' >&2
    return 127
  }
...
plan_rules() {
  [ -s "$STATE_DIR/ips-v4" ] && [ -s "$STATE_DIR/ips-v6" ] || { echo 'TNA_CF_CIDR_ERROR=FETCH_FIRST' >&2; return 123; }
  local ssh_port="${2:-22}"
  case "$ssh_port" in ''|*[!0-9]*) echo 'TNA_CF_CIDR_ERROR=SSH_PORT_INVALID' >&2; return 123;; esac
  printf '__TNA_CF_ORIGIN_LOCK_PLAN_BEGIN__\n'
  printf 'KEEP_SSH_PORT=%s\n' "$ssh_port"
  printf 'KEEP_PUBLIC_TCP_443_UNCHANGED=1\n'
  print_443_observation
  ...
  printf 'PLAN_ONLY=1\n'
  printf '__TNA_CF_ORIGIN_LOCK_PLAN_END__\n'
}
