...
Windows EXE 在主菜单前一次性检查/安装并验证 OpenSSH；Windows 菜单 `[14]` 可在不登录 VPS 的情况下配置、撤销或查看当前用户的本地 `127.0.0.1:10808` 代理环境变量。macOS 原生 GUI/CLI 的 `[14]` 管理系统级 HTTP/HTTPS/SOCKS 代理并关闭 PAC/WPAD：配置前保存当前设置，撤销或恢复时还原原设置，必要时只请求 macOS 管理员授权，不读取或连接 VPS。
...
每项远端操作都会把最近的非敏感目标（节点 IP/主机名、SSH 用户和端口）自动载入表单供确认/修改，不会自动执行；没有历史时保持空白。认证方式仍需明确选择，只有存在匹配且已绑定的长期 key 时才默认选中。历史可随时单条删除或清空，不保存密码、域名、邮箱或私钥。
...
3  自动体检与排障
4  安全自动修复
5  VPS 登录密码：随机 / 自定义 / 取消
6  3x-ui 账号密码：随机 / 自定义 / 取消
7  显示/复制凭据交接单
8  优化前台伪装网站 + Nginx
9  节点备份
10 紧急诊断报告
...
除 `[12]`、`[14]`、`[T]`、`[H]`、语言和退出等纯本机项目外，每个远端项目都会打开连接表单：最近目标会先载入但可修改；没有匹配长期 key 时必须明确选择临时密码或长期 key，不会静默执行或把密码带入下一次操作。
...
# 密码/密钥：随机或自定义，必须让操作者看到真实值

菜单 `[5]` 和 `[6]` 都先要求明确选择：`1` 随机生成、`2` 自定义、`0` 取消。自定义值在遮罩输入框中二次确认；确认前不写远端，取消直接返回。密码只在本次 SSH 操作中通过随机命名、root-only、`0600` 的一次性文件传递，脚本和客户端会在成功、失败、取消或中断时清理，不写入设置、命令行或普通日志。

选择随机并成功完成验证时，v1.0.0 会在交接单中展示：
...
这样高强度随机密码不要求人脑记；自定义密码也只在完整交接单通过校验后显示/复制。
