...
for state_copy in \
  "$PAYLOAD/root/.config/proxy-runbook" \
  "$PAYLOAD/root/.config/text-node-assistant"; do
  find "$state_copy" -maxdepth 1 -type f \
...
{
  printf 'TEXT_NODE_CURRENT_CONFIG_BACKUP=1\n'
...
grep -Fxq 'TEXT_NODE_CURRENT_CONFIG_BACKUP=1' "$VERIFY/current-config/BACKUP_INFO.txt"
if find "$VERIFY/current-config" -type f \
  \( -name 'xray-template-before-warp-*.json' \
     -o -name 'proxy-node-backup-*' \
...
mapfile -d '' targets < <(
  {
    find /root -xdev -mindepth 1 -maxdepth 1 \
      \( -name 'proxy-node-backup-*' \
         -o -name 'text-node-backup-*' \
         -o -name 'proxy-node-current-config-*.tar.gz' \
      ...
      ! -path "$ARCHIVE" -print0
    find /root/.config/proxy-runbook -xdev -mindepth 1 -maxdepth 1 -type f \
...
for target in "${targets[@]}"; do
  case "$target" in
    /root/proxy-node-backup-*|\
    /root/text-node-backup-*|\
    /root/proxy-node-current-config-*.tar.gz|\
...
current_count="$(find /root -xdev -mindepth 1 -maxdepth 1 -type f \
  \( -name 'proxy-node-current-config-*.tar.gz' -o -name 'text-node-current-config-*.tar.gz' \) | wc -l)"
[ "$current_count" -eq 1 ] || {
