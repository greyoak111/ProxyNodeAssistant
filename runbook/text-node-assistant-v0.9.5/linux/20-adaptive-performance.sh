...
BACKUP_ROOT="/root/text-node-assistant-performance-backups"
SYSCTL_OLD="/etc/sysctl.d/99-text-node-assistant.conf"
SYSCTL_FILE="/etc/sysctl.d/99-text-node-assistant-performance.conf"
XUI_OVERRIDE="/etc/systemd/system/x-ui.service.d/90-text-node-assistant-performance.conf"
NGINX_OVERRIDE="/etc/systemd/system/nginx.service.d/90-text-node-assistant-performance.conf"
NGINX_CONF="/etc/nginx/nginx.conf"
ZRAM_UNIT="/etc/systemd/system/text-node-assistant-zram.service"
...
create_backup() {
  ...
  if systemctl is-active --quiet text-node-assistant-zram.service 2>/dev/null; then
...
restore_backup() {
  ...
  systemctl disable --now text-node-assistant-zram.service >/dev/null 2>&1 || true
  ...
  if [ -f "$backup/zram.was-active" ] && [ -f "$ZRAM_UNIT" ]; then
    systemctl enable --now text-node-assistant-zram.service >/dev/null 2>&1 || true
  fi
...
write_zram_unit() {
  ...
  cat > "$ZRAM_UNIT" <<EOF
[Unit]
Description=TextNodeAssistant bounded low-memory zram swap
...
apply_profile() {
  ...
  {
    echo '# Managed by TextNodeAssistant v0.9.5. Use menu [16] to change or roll back.'
  ...
  if [ "$ZRAM_MB" -gt 0 ] && command -v zramctl >/dev/null 2>&1 && modprobe zram 2>/dev/null; then
    ...
    systemctl daemon-reload
    systemctl enable --now text-node-assistant-zram.service >/dev/null
