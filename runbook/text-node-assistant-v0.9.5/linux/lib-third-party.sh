...
tna_download_copyparty_pinned() (
  ...
  tna_sha256_check "$COPYPARTY_SFX_SHA256" "$tmp" || {
    echo "TNA_SUPPLY_CHAIN_ERROR=COPYPARTY_HASH" >&2
    exit 75
  }
  actual_size="$(wc -c < "$tmp" | tr -d '[:space:]')"
  [ "$actual_size" = "$COPYPARTY_SFX_SIZE" ] || {
    echo "TNA_SUPPLY_CHAIN_ERROR=COPYPARTY_SIZE" >&2
    exit 75
  }
  install -D -o root -g root -m 0755 "$tmp" "$destination"
  printf 'TNA_COPYPARTY_PINNED_DOWNLOAD_OK version=%s sha256=%s\n' "$COPYPARTY_VERSION" "$COPYPARTY_SFX_SHA256"
)
