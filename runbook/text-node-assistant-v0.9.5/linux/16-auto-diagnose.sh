...
PUB="/etc/text-node-assistant/public.env"
[ -f "$PUB" ] && . "$PUB"
...
if grep -q '^PRIVATE_DRIVE_MODE=copyparty$' /etc/text-node-assistant/private-drive.env 2>/dev/null; then
  DRIVE_PORT="$(sed -n 's/^COPYPARTY_LOOPBACK_PORT=//p' /etc/text-node-assistant/private-drive.env | sed -n '1p')"
  DRIVE_LISTEN=''
  if [[ "$DRIVE_PORT" =~ ^39[0-9]{3}$ ]]; then
    DRIVE_LISTEN="$(ss -H -lntp 2>/dev/null | awk -v p="127.0.0.1:$DRIVE_PORT" '$4 == p {print $4}')"
  fi
  if systemctl is-active --quiet text-node-assistant-copyparty.service 2>/dev/null; then
  ...
  if [ -n "$DRIVE_LISTEN" ] && [ "$DRIVE_LISTEN" = "127.0.0.1:$DRIVE_PORT" ]; then
    pass PRIVATE_DRIVE_LOOPBACK "网盘内核只监听动态回环端口 $DRIVE_PORT" "The drive engine listens only on dynamic loopback port $DRIVE_PORT"
  else
    add_issue PRIVATE_DRIVE_LISTENER_BAD ERROR "网盘动态回环监听缺失或超出回环范围；已阻止完成状态" \
      "The drive listener is missing or not loopback-only; public-ready status is blocked" REBUILD_PRIVATE_DRIVE false
  fi
  if [ -r /etc/text-node-assistant/copyparty.conf ] && \
     grep -q '^  i: 127.0.0.1$' /etc/text-node-assistant/copyparty.conf && \
     ! grep -qE '^[[:space:]]+[A-Za-z.]+:[[:space:]]+\*$' /etc/text-node-assistant/copyparty.conf; then
  ...
  add_issue PRIVATE_DRIVE_MISSING ERROR "强制私人网盘未安装；请从唯一安装入口 [1] 收敛" \
    "The mandatory private drive is missing; converge it through the only install entry [1]" INSTALL_MANDATORY_DRIVE false
fi
...
DEVICE_REGISTRY="/etc/text-node-assistant/device-registry.json"
if [ -s "$DEVICE_REGISTRY" ]; then
  ...
  if jq -e --arg node "$NODE_ID_CURRENT" '.version==1 and .nodeId==$node and (.devices|type=="array") and (.invites|type=="array")' "$DEVICE_REGISTRY" >/dev/null 2>&1 && \
     [ -n "$INBOUND_LIST" ] && jq -e --slurpfile registry "$DEVICE_REGISTRY" '
       def managed: (.comment // "") | startswith("tna-device:");
       def marker($id): "tna-device:" + $id;
	   ...
	   (([$inbounds.obj[]? | select((.remark=="tna-cdn-xhttp" or .remark=="pna-cdn-xhttp") and .protocol=="vless" and .streamSettings.network=="xhttp")] | length)==0 or
        all($registry[0].devices[]? | select(.status!="revoked") as $device;
		  any($inbounds.obj[]? | select((.remark=="tna-cdn-xhttp" or .remark=="pna-cdn-xhttp") and .protocol=="vless" and .streamSettings.network=="xhttp") | .settings.clients[]?;
...
IP_REBIND_STATE="/etc/text-node-assistant/ip-rebind-public.env"
...
if [ -n "${COVER_DOMAIN:-}" ]; then
  ...
  SUB_PROXY_LISTEN="$(ss -lntp 2>/dev/null | grep -E ':2097[[:space:]]' || true)"
  if grep -q '127.0.0.1:2097' <<<"$SUB_PROXY_LISTEN" && \
     nginx -T 2>/dev/null | grep -F 'proxy_pass http://127.0.0.1:2097;' >/dev/null; then
    pass SUBSCRIPTION_PROXY "HTTPS 伪装站已通过本地适配器反代订阅路径" "The HTTPS cover site proxies subscriptions through the local adapter"
  else
  ...
  if [ -n "$SUB_ID" ]; then
    ...
    SUB_TEXT="$(printf '%s' "$SUB_BODY" | base64 -d 2>/dev/null || true)"
    [ -n "$SUB_TEXT" ] || SUB_TEXT="$SUB_BODY"
    SUB_XHTTP="$(printf '%s' "$SUB_TEXT" | grep -i -m1 'type=xhttp' || true)"
    if [ -n "$SUB_XHTTP" ]; then
      if grep -qi 'security=tls' <<<"$SUB_XHTTP" && \
         grep -qi 'sni=' <<<"$SUB_XHTTP" && \
         grep -qi 'host=' <<<"$SUB_XHTTP" && \
         grep -qi 'mode=packet-up' <<<"$SUB_XHTTP"; then
        pass SUBSCRIPTION_PUBLIC "公网订阅可下载，XHTTP 节点已带 TLS/SNI/Host" "The public subscription downloads with repaired TLS/SNI/Host XHTTP metadata"
      else
        add_issue SUBSCRIPTION_PUBLIC_BAD ERROR "公网订阅中的 XHTTP 缺少 TLS/SNI/Host；客户端会显示 -1" \
          "The public subscription XHTTP link is missing TLS/SNI/Host and clients may show latency -1" NORMALIZE_SUBSCRIPTION true
      fi
    else
      SUB_LINK="$(printf '%s' "$SUB_TEXT" | sed -n '/^vless:\/\//p' | sed -n '1p')"
      SUB_HOST="${SUB_LINK#*@}"
      SUB_HOST="${SUB_HOST%%:*}"
  ...
  if [ -f /var/www/cover/.text-node-assistant-cover ]; then
...
if [ -s /root/.config/text-node-assistant/performance-profile.env ]; then
  PROFILE="$(sed -n 's/^PROFILE=//p' /root/.config/text-node-assistant/performance-profile.env | sed -n '1p')"
