...
managed_reference_count() {
  ...
  for file in "$PUBLIC_FILE" "$IDENTITY_FILE" "$DEPLOYMENT_FILE" \
    /etc/text-node-assistant/cdn-xhttp.env /etc/text-node-assistant/private-drive.env \
...
unmanaged_reference_count() {
  ...
  while IFS= read -r file; do
    case "$file" in
      /etc/text-node-assistant/*|/etc/nginx/conf.d/text-node-assistant*|/etc/nginx/sites-enabled/text-node-assistant*) continue;;
    esac
    count=$((count + 1))
...
create_snapshot() {
  ...
  dir="/root/text-node-ip-rebind-${stamp}"
  archive="${dir}.tar.gz"
  install -d -m 700 "$dir"
  for path in /etc/text-node-assistant /etc/x-ui /etc/nginx /etc/letsencrypt /etc/ufw /etc/fail2ban; do
...
preflight() {
  ...
  echo '__TNA_IP_REBIND_PREFLIGHT_V1_BEGIN__'
  ...
  echo '__TNA_IP_REBIND_PREFLIGHT_V1_END__'
}
...
commit_direct() {
  ...
  echo '__TNA_IP_REBIND_COMMIT_V1_BEGIN__'
  ...
  echo '__TNA_IP_REBIND_COMMIT_V1_END__'
}
...
wait_cloudflare() {
  ...
  echo '__TNA_IP_REBIND_WAIT_V1_BEGIN__'
  ...
  echo '__TNA_IP_REBIND_WAIT_V1_END__'
}
...
show_status() {
  if [ ! -s "$STATE_FILE" ]; then
    echo '__TNA_IP_REBIND_STATUS_V1_BEGIN__'
    echo 'IP_REBIND_STATUS=IDLE'
    echo '__TNA_IP_REBIND_STATUS_V1_END__'
    return
  fi
  echo '__TNA_IP_REBIND_STATUS_V1_BEGIN__'
