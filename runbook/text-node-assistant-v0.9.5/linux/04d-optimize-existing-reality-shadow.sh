...
prepare() {
  ...
  jq -nc \
    ...
    > /root/.config/text-node-assistant/existing-reality-shadow.json
  chmod 600 /root/.config/text-node-assistant/existing-reality-shadow.json

  if [ -n "${SSH_CONNECTION:-}" ]; then
    SRC="${SSH_CONNECTION%% *}"
    ufw allow from "$SRC" to any port "$TEST_PORT" proto tcp comment 'text-node-assistant-existing-reality-shadow' >/dev/null || true
  fi
...
commit() {
  ...
  mapfile -t N < <(ufw status numbered | grep "$TEST_PORT" | grep 'text-node-assistant-existing-reality-shadow' \
    | sed -n 's/^\[[[:space:]]*\([0-9]\+\)\].*/\1/p' | sort -rn)
  for n in "${N[@]:-}"; do [ -n "$n" ] && yes | ufw delete "$n" >/dev/null || true; done

  rm -f /root/.config/text-node-assistant/existing-reality-shadow.json
