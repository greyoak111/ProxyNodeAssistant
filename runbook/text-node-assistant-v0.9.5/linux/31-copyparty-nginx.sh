...
TEMPLATE="$ROOT/templates/nginx/text-node-assistant-copyparty.conf.in"
CANDIDATE_DIR=/etc/text-node-assistant/candidates
AVAILABLE=/etc/nginx/sites-available/tna-private-drive
ENABLED=/etc/nginx/sites-enabled/tna-private-drive

[ "$(id -u)" -eq 0 ] || { echo 'TNA_DRIVE_NGINX_ERROR=ROOT_REQUIRED' >&2; exit 151; }
...
render() {
  ...
  valid_hostname "$hostname" || { echo 'TNA_DRIVE_NGINX_ERROR=HOSTNAME_INVALID' >&2; return 152; }
  valid_port "$port" || { echo 'TNA_DRIVE_NGINX_ERROR=PORT_NOT_ALLOWLISTED' >&2; return 152; }
  tmp="$(mktemp "$(dirname "$destination")/.tna-drive-nginx.XXXXXX")"
  sed -e "s|@DRIVE_HOSTNAME@|${hostname}|g" -e "s|@ORIGIN_PORT@|${port}|g" -e "s|@LISTEN_ADDRESS@|${address}|g" "$TEMPLATE" > "$tmp"
  grep -qF '# TNA_MANAGED_COPYPARTY_NGINX_V095' "$tmp" || { rm -f -- "$tmp"; return 153; }
  chmod 0600 "$tmp"
...
prepare() {
  ...
  echo 'TNA_DRIVE_NGINX_CANDIDATE_READY'
  echo 'TNA_DRIVE_NGINX_NOT_ENABLED=WAITING_FOR_CLOUDFLARE_AND_CERTIFICATE'
}

stage_local() {
  local hostname="$1" port="$2"
  [ -s "/etc/letsencrypt/live/${hostname}/fullchain.pem" ] && [ -s "/etc/letsencrypt/live/${hostname}/privkey.pem" ] || {
    echo 'TNA_DRIVE_NGINX_ERROR=CERTIFICATE_MISSING' >&2; return 154;
  }
  systemctl is-active --quiet text-node-assistant-copyparty.service || { echo 'TNA_DRIVE_NGINX_ERROR=COPYPARTY_INACTIVE' >&2; return 154; }
  if [ -e "$AVAILABLE" ] && ! grep -qF '# TNA_MANAGED_COPYPARTY_NGINX_V095' "$AVAILABLE"; then
    echo 'TNA_DRIVE_NGINX_ERROR=UNMANAGED_CONFIG_EXISTS' >&2; return 155
  fi
  ...
  ss -H -lntp 2>/dev/null | awk -v target="127.0.0.3:${port}" '$4 == target {found=1} END{exit found ? 0 : 1}' || {
    echo 'TNA_DRIVE_NGINX_ERROR=LOCAL_LISTENER_MISSING' >&2; return 157;
  }
  echo 'TNA_DRIVE_NGINX_LOCAL_STAGE_READY'
  echo 'TNA_DRIVE_PUBLIC_ACCESS=BLOCKED'
}
