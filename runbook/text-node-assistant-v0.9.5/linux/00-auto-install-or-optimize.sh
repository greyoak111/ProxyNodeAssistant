...
PRIVATE_DIR="/root/.config/text-node-assistant"
...
on_run_exit() {
  ...
  if [ "$rc" -ne 0 ] && [ "$RUN_COMPLETE" -ne 1 ]; then
    ...
    printf '\nTNA_REMOTE_FAILURE stage=%s rc=%s\n' "$CURRENT_STAGE" "$rc" >&2
  fi
}
...
AUTO_DEFAULTS="${TNA_ASSUME_DEFAULTS:-0}"
RUN_LANG="${TNA_LANG:-en}"
COVER_TEMPLATE_CHOICE="${TNA_COVER_TEMPLATE:-auto}"
TOPOLOGY_MODE="${TNA_TOPOLOGY_MODE:-gray}"
case "$TOPOLOGY_MODE" in gray|orange|dual) ;; *) red "Invalid topology mode."; exit 1;; esac
...
LOGIN_USER="${TNA_LOGIN_USER:-${SUDO_USER:-root}}"
...
if [ "$TOPOLOGY_MODE" = orange ]; then
  if ! getent ahostsv4 "$DOMAIN" 2>/dev/null | awk 'NF {found=1} END{exit found ? 0 : 1}'; then
    red "Orange-cloud hostname has no IPv4 answer. Stop before ACME."
    exit 2
  fi
  green "DNS_ORANGE_PROXIED_ACCEPTED — the Windows controller already verified public resolver quorum"
...
step "REALITY 443"
if [ "$TOPOLOGY_MODE" = orange ]; then
  yellow "ORANGE_ONLY_REALITY_SKIPPED — this topology never creates a temporary 24443 or production Reality route."
else
  set +e
  ...
  if [ "$RC" -eq 0 ]; then
    ...
    if yesq "No VLESS+REALITY 443 exists. Create a safe 24443 shadow first?"; then
      ...
      if [ -n "${SSH_SOURCE:-}" ]; then
        ufw allow from "$SSH_SOURCE" to any port 24443 proto tcp comment 'text-node-assistant-reality-shadow'
      else
      ...
      if human_yesq "你已经亲自把 24443 链接导入客户端，并确认能正常上网了吗？" "Have you personally imported the 24443 link and verified real browsing works?"; then
        ...
        mapfile -t N < <(ufw status numbered | grep '24443' | grep 'text-node-assistant-reality-shadow' \
          | sed -n 's/^\[[[:space:]]*\([0-9]\+\)\].*/\1/p' | sort -rn)
...
[ ! -x /usr/local/lib/text-node-assistant/security-firewall.sh ] || \
  /usr/local/lib/text-node-assistant/security-firewall.sh restart || true
