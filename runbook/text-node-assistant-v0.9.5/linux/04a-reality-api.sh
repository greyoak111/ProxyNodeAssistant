...
create_test() {
  ...
  payload="$(jq -nc \
    ...
    '{
      ...
      settings:{
        clients:[{id:$uuid,email:$email,flow:"xtls-rprx-vision",limitIp:0,totalGB:0,expiryTime:0,enable:true,tgId:0,subId:$sub,comment:"text-node-assistant-v0.6"}],
  ...
  } > /root/.config/text-node-assistant/reality-shadow.env
  chmod 600 /root/.config/text-node-assistant/reality-shadow.env
  ...
  echo "Root-only copy: /root/.config/text-node-assistant/reality-shadow.env"
...
promote_shadow() {
  ...
  echo "$link" > /root/text-node-client-link.txt
  chmod 600 /root/text-node-client-link.txt
  ...
  echo "$link"
  echo
  echo "Saved root-only: /root/text-node-client-link.txt"
}

show_shadow() {
  local port="${1:-24443}" state="/root/.config/text-node-assistant/reality-shadow.env"
  local saved_port link obj
  [ -s "$state" ] || return 1
  # create-test has historically persisted TEST_PORT.  A short-lived v0.9.5
...
delete_production() {
  local obj id resp
  obj="$(get_by_port 443)"
  [ -n "$obj" ] || { echo 'REALITY_443_NOT_INSTALLED'; return 0; }
  [ "$(jq -r '.protocol // empty' <<<"$obj")" = vless ] && \
    [ "$(jq -r '.streamSettings.security // empty' <<<"$obj")" = reality ] && \
    [ "$(jq -r '.streamSettings.realitySettings.target // .streamSettings.realitySettings.dest // empty' <<<"$obj")" = 127.0.0.1:8443 ] || {
      echo 'ERROR: port 443 is not the exact managed Reality origin; refusing deletion.' >&2; return 1;
    }
  id="$(jq -r '.id' <<<"$obj")"
  resp="$(xui_auth_curl -X POST "${XUI_BASE}/panel/api/inbounds/del/${id}")"
  jq -e '.success == true' <<<"$resp" >/dev/null || { jq '{success,msg}' <<<"$resp" >&2; return 1; }
  [ -z "$(get_by_port 443)" ] || { echo 'ERROR: Reality 443 deletion readback failed.' >&2; return 1; }
  rm -f -- /root/text-node-client-link.txt /root/.config/text-node-assistant/reality-shadow.env
  for key in REALITY_PRODUCTION_LINK REALITY_TEST_LINK REALITY_TEST_UUID REALITY_PRIVATE_KEY REALITY_PUBLIC_KEY REALITY_SHORT_ID REALITY_TEST_SUB_ID; do handoff_delete "$key"; done
  echo 'REALITY_443_DELETED'
}
...
case "$cmd" in
  ...
  delete-production) [ "$#" -eq 0 ] || exit 2; delete_production ;;
  ...
  *) echo "usage: $0 {create-test DOMAIN PUBLIC_IP [PORT]|show-shadow [PORT]|promote-shadow [TEST_PORT] [PROD_PORT]|inspect-443 [EXPECTED_DOMAIN] [EXPECTED_IP]|normalize-share PUBLIC_IP|delete-production|list}"; exit 1 ;;
esac
