...
die() {
  printf 'PNA_NODE_IDENTITY_ERROR=%s\n' "$1" >&2
  exit "${2:-1}"
}
...
validate_state() {
  ...
  [[ "$(value SERVER_ID)" =~ ^pna-srv-[0-9a-f]{32}$ ]] || die SERVER_ID_INVALID 54
  [[ "$(value NODE_ID)" =~ ^pna-node-[0-9a-f]{32}$ ]] || die NODE_ID_INVALID 54
...
init_state() {
  ...
  if [ -s "$STATE_FILE" ]; then
    validate_state
    show_state
    echo 'PNA_NODE_IDENTITY_ALREADY_INITIALIZED'
    return
  fi
  ...
  tmp="$(mktemp "$STATE_DIR/.node-identity.XXXXXX")"
  {
    ...
    printf 'SERVER_ID=pna-srv-%s\n' "$(openssl rand -hex 16)"
    printf 'NODE_ID=pna-node-%s\n' "$(openssl rand -hex 16)"
  ...
  validate_state
  show_state
  echo 'PNA_NODE_IDENTITY_INITIALIZED'
}
...
show_state() {
  ...
  echo '__PNA_NODE_IDENTITY_V1_BEGIN__'
  printf 'SERVER_ID=%s\n' "$(value SERVER_ID)"
  printf 'NODE_ID=%s\n' "$(value NODE_ID)"
  ...
  echo '__PNA_NODE_IDENTITY_V1_END__'
}
