...
nginx -T 2>/dev/null | grep -F "proxy_pass http://127.0.0.1:${SUB_PORT};" >/dev/null || {
  echo "ERROR: Nginx HTTPS subscription reverse proxy is missing."
  exit 1
}

STATUS="$(curl -sS --max-time 10 -o /dev/null -w '%{http_code}' \
  -H "Host: ${DOMAIN}" "http://127.0.0.1:${SUB_PORT}/sub/proxy-runbook-probe-missing")"
[ "$STATUS" = "404" ] || {
  echo "ERROR: localhost subscription probe returned HTTP ${STATUS}."
  exit 1
}

echo "SUBSCRIPTION_PROXY_OPTIMAL uri=${SUB_URI}<client-sub-id>"
