...
if [ -x /opt/proxy-runbook-current/linux/20-adaptive-performance.sh ]; then
  bash /opt/proxy-runbook-current/linux/20-adaptive-performance.sh --status || true
else
  echo "PERFORMANCE_PROFILE_UNAVAILABLE"
fi

section TRAFFIC_COUNTER
if [ -x /opt/proxy-runbook-current/linux/21-traffic-status.sh ]; then
  bash /opt/proxy-runbook-current/linux/21-traffic-status.sh --status || true
else
  echo "TRAFFIC_COUNTER_UNAVAILABLE"
fi

section IP_REBIND_TRANSACTION
if [ -x /opt/proxy-runbook-current/linux/27-ip-rebind.sh ]; then
  bash /opt/proxy-runbook-current/linux/27-ip-rebind.sh status || true
else
