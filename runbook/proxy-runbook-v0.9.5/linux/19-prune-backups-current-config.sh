...
STAGE="$(mktemp -d /root/.proxy-node-current-config.XXXXXX)"
VERIFY="$(mktemp -d /root/.verify-current-config.XXXXXX)"
PAYLOAD="$STAGE/current-config"
ARCHIVE="/root/proxy-node-current-config-${STAMP}.tar.gz"
...
find "$PAYLOAD/root/.config/proxy-runbook" -maxdepth 1 -type f \
...
current_count="$(find /root -xdev -mindepth 1 -maxdepth 1 -type f \
  -name 'proxy-node-current-config-*.tar.gz' | wc -l)"
[ "$current_count" -eq 1 ] || {
