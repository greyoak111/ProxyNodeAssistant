...
managed_paths=(
  ...
  /etc/proxy-node-assistant
  /opt/proxy-node-assistant/copyparty
  /srv/proxy-node-assistant/drive-data
  /var/lib/proxy-node-assistant/copyparty
  /var/log/proxy-node-assistant/copyparty
  /etc/systemd/system/proxy-node-assistant-copyparty.service
  /usr/local/lib/proxy-node-assistant/security-firewall.sh
  /etc/systemd/system/proxy-node-assistant-security-firewall.service
  /etc/logrotate.d/proxy-node-assistant-security
  /etc/nginx/conf.d/proxy-node-assistant-security-log.conf
)
...
owned_services=(nginx warp-svc vnstat x-ui proxy-runbook-zram.service proxy-node-assistant-copyparty.service proxy-node-assistant-security-firewall.service)
...
plan() {
  ...
  print_presence HAS_TOOLKIT test -e /opt/proxy-runbook-current
  print_presence HAS_PRIVATE_DRIVE_DATA test -e /srv/proxy-node-assistant/drive-data
  print_presence HAS_COPYPARTY_SERVICE test -e /etc/systemd/system/proxy-node-assistant-copyparty.service
...
remove_current_stack() {
  local domain="" package purge_packages=()
  if [ -x /opt/proxy-runbook-current/linux/24-security-baseline.sh ]; then
    bash /opt/proxy-runbook-current/linux/24-security-baseline.sh --remove || true
  elif [ -x /opt/proxy-runbook-v0.9.5/linux/24-security-baseline.sh ]; then
    bash /opt/proxy-runbook-v0.9.5/linux/24-security-baseline.sh --remove || true
  fi
  ...
  rm -f -- /etc/nginx/sites-enabled/pna-private-drive /etc/nginx/sites-available/pna-private-drive
  rm -rf -- /etc/proxy-node-assistant /opt/proxy-node-assistant/copyparty
  rm -rf -- /srv/proxy-node-assistant/drive-data /var/lib/proxy-node-assistant/copyparty /var/log/proxy-node-assistant/copyparty
  rm -f -- /etc/systemd/system/proxy-node-assistant-copyparty.service
  rm -f -- /var/log/nginx/proxy-node-assistant-security.log /var/log/nginx/proxy-node-assistant-security.log.*
  rm -f -- /etc/nginx/conf.d/proxy-node-assistant-security-log.conf
...
remove_tool_data_last() {
  ...
  rm -f -- /root/proxy-node-backup-*.tar.gz /root/proxy-node-current-config-*.tar.gz
  if [ "$(baseline_mode)" != "EXACT" ]; then
  ...
  rm -f -- /usr/local/sbin/proxy-node /usr/local/bin/proxy-node
  rm -f -- /opt/proxy-runbook-current
  rm -rf -- \
    ...
    /opt/proxy-runbook-v0.9.0 /opt/proxy-runbook-v0.9.5
  ...
  rm -f -- /tmp/proxy-runbook-toolkit-v0.8.*.tar.gz /tmp/proxy-runbook-toolkit-v0.9.0.tar.gz /tmp/proxy-runbook-toolkit-v0.9.5.tar.gz
