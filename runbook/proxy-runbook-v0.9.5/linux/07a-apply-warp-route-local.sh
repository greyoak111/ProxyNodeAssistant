...
printf '%s\n' "$OUTER" > "/root/.config/proxy-runbook/xray-template-before-warp-${STAMP}.json"
chmod 600 "/root/.config/proxy-runbook/xray-template-before-warp-${STAMP}.json"
...
echo "Backup: /root/.config/proxy-runbook/xray-template-before-warp-${STAMP}.json"
sleep 3
