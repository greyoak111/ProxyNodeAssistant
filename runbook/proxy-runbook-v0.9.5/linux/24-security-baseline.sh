...
JAIL_FILE="/etc/fail2ban/jail.d/proxy-node-assistant-sshd.local"
HELPER_DIR="/usr/local/lib/proxy-node-assistant"
FIREWALL_HELPER="$HELPER_DIR/security-firewall.sh"
FIREWALL_UNIT="/etc/systemd/system/proxy-node-assistant-security-firewall.service"
LOGROTATE_FILE="/etc/logrotate.d/proxy-node-assistant-security"
JAIL_MARKER="# PNA_MANAGED_FAIL2BAN_SSHD_V095"
HELPER_MARKER="# PNA_MANAGED_SECURITY_FIREWALL_V095"
UNIT_MARKER="# PNA_MANAGED_SECURITY_FIREWALL_UNIT_V095"
LOGROTATE_MARKER="# PNA_MANAGED_SECURITY_LOGROTATE_V095"

die() {
  printf 'PNA_SECURITY_BASELINE_ERROR=%s\n' "$1" >&2
  exit "${2:-1}"
}
...
write_firewall_helper() {
  ...
  cat > "$tmp" <<'EOF'
#!/usr/bin/env bash
# PNA_MANAGED_SECURITY_FIREWALL_V095
set -Eeuo pipefail

COMMENT="PNA_REALITY_CONN_V095"
PREFIX="PNA-REALITY "

is_direct_reality() {
  grep -q '^PORT_443_OWNER=xray-reality$' /etc/proxy-runbook/deployment-mode.env 2>/dev/null ||
...
write_firewall_unit() {
  ...
  tmp="$(mktemp)"
  cat > "$tmp" <<'EOF'
# PNA_MANAGED_SECURITY_FIREWALL_UNIT_V095
[Unit]
Description=ProxyNodeAssistant privacy-preserving connection metadata rule
After=network-online.target ufw.service
Wants=network-online.target

[Service]
Type=oneshot
ExecStart=/usr/local/lib/proxy-node-assistant/security-firewall.sh start
ExecStop=/usr/local/lib/proxy-node-assistant/security-firewall.sh stop
RemainAfterExit=yes
...
write_logrotate() {
  ...
  cat > "$tmp" <<EOF
$LOGROTATE_MARKER
/var/log/nginx/proxy-node-assistant-security.log {
    daily
...
apply_baseline() {
  ...
  systemctl enable --now proxy-node-assistant-security-firewall.service >/dev/null || die FIREWALL_METADATA_UNIT_START_FAILED 47
  "$FIREWALL_HELPER" restart
  echo 'PNA_SECURITY_BASELINE_APPLIED'
  status_baseline
}
...
status_baseline() {
  ...
  echo 'PNA_SECURITY_BASELINE_STATUS_BEGIN'
  ...
  echo 'PNA_SECURITY_BASELINE_STATUS_END'
}
...
remove_baseline() {
  ...
  systemctl disable --now proxy-node-assistant-security-firewall.service >/dev/null 2>&1 || true
  ...
  echo 'PNA_SECURITY_BASELINE_REMOVED'
