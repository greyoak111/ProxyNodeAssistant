...
RETENTION_DAYS="$(sed -n 's/^RETENTION_DAYS=//p' /etc/proxy-runbook/security-baseline.env 2>/dev/null | sed -n '1p')"
RETENTION_DAYS="${RETENTION_DAYS:-7}"
if [ -x "$ROOT/linux/24-security-baseline.sh" ]; then
  bash "$ROOT/linux/24-security-baseline.sh" --apply "$RETENTION_DAYS" || true
fi
if grep -q '^PRIVATE_DRIVE_MODE=copyparty$' /etc/proxy-runbook/private-drive.env 2>/dev/null && \
   [ -s /etc/proxy-node-assistant/copyparty.conf ] && \
   [ -s /opt/proxy-node-assistant/copyparty/copyparty-sfx.py ] && \
   [ -s /etc/systemd/system/proxy-node-assistant-copyparty.service ]; then
  systemctl enable --now proxy-node-assistant-copyparty.service 2>/dev/null || true
fi
