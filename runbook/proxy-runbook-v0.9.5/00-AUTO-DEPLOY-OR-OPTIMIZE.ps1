...
$archive = Join-Path $here "proxy-runbook-toolkit-v0.9.5.tar.gz"
...
& scp -i $keyPath -P $SshPort $archive "$SshUser@${VpsHost}:/tmp/proxy-runbook-toolkit-v0.9.5.tar.gz"
if ($LASTEXITCODE -ne 0) { throw "SCP upload failed." }

$isRoot = $SshUser -eq "root"
$extract = 'mkdir -p /opt; rm -rf /opt/proxy-runbook-v0.9.5; tar -xzf /tmp/proxy-runbook-toolkit-v0.9.5.tar.gz -C /opt; bash /opt/proxy-runbook-v0.9.5/linux/00-bootstrap-toolkit.sh'
