# TextNodeAssistant v0.9.5

面向 Windows 与 Android 的本地优先 VPS 节点部署、维护、排障与恢复工具。

> 本仓库的 v0.9.5 是“重置线”：以稳定的 v0.9.0 为代码基线重新迭代，不继承旧实验版 v0.9.5 的设备准入、controller、邀请、网盘或本机 admin。版本号相同，但内部构建修订更高，菜单 `[1]` 能识别并安全替换旧实验构建。

[完整使用说明书](TextNodeAssistant-v0.9.5-完整使用说明书.md) · [从零部署教程](TextNodeAssistant-v0.9.5-从零部署教程.md) · [更新说明](TextNodeAssistant-v0.9.5-更新说明.md) · [Android 手册](ANDROID.md) · [构建说明](BUILD.md) · [MIT License](LICENSE)

## 重置后的产品边界

TextNodeAssistant 只做一件事：在本人拥有或获得明确授权的 VPS 上，以可审计、可回滚、失败即停的方式管理节点。

重置线彻底取消：

- 设备准入、controller/member 角色、邀请、租约、围栏和“受信设备才能管理”的门限；
- 私人网盘、Copyparty、普通网盘账户、本机 admin、恢复托管和相关界面；
...
- 15 套本地伪装模板、中英双语、历史 VPS 快速选择和密钥归档/恢复。

SSH key 只是这台电脑/手机登录某台 VPS 的凭据，不再被包装成设备身份系统。
...
| 5 / 6 / 7 | VPS 密码、面板身份、交接单 | 真凭据经完整性验证后才显示/复制 |
...
完整二级菜单、输入词和失败分支见[完整使用说明书](TextNodeAssistant-v0.9.5-完整使用说明书.md)。

## 从 v0.9.0 或旧实验版 v0.9.5 升级

- v0.9.0 或更旧：菜单 `[1]` 识别旧版、先备份、升级工具包并按新计划收敛；
- 旧实验版 v0.9.5：可见版本相同，但内部 revision 低于重置线；菜单 `[1]` 会执行受控替换并退役设备门限/网盘组件；
- 与当前 EXE/APK 完全同构建：禁止重复上传和 bootstrap；
- 远端内部 revision 更新：拒绝降级，提示更换更新的客户端。

退役旧实验功能时，会停止并移除受管服务与程序，但不会把旧网盘用户文件根当垃圾直接删除。保留的数据不再由重置版管理，应先离线核对/备份，再由本人决定后续处理。
