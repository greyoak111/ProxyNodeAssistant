...
- `[12]` 清空剪贴板、`[14]` 本地 10808、`[T]` 服务商流量中心、`[H]` 登录历史和 `[K]` 多 key 管理均是本机操作；这些工作区只显示“仅本机”说明，不展示 VPS、SSH 用户、端口或密码字段，也不会发起 SSH；
...
2. 打开应用的“设置”，点击“无管理员卸载”。它会退出应用并清理应用自身目录和用户级配置，不弹管理员授权。
